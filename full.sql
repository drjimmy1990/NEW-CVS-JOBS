...
CREATE POLICY "Users can delete own cv sessions" ON public.cv_sessions FOR DELETE USING (auth.uid () = user_id);
...
CREATE POLICY "Users can view own practice sessions" ON public.interview_practice_sessions FOR
SELECT USING (auth.uid () = user_id);

CREATE POLICY "Users can insert own practice sessions" ON public.interview_practice_sessions FOR
INSERT
WITH
    CHECK (auth.uid () = user_id);

CREATE POLICY "Users can update own practice sessions" ON public.interview_practice_sessions FOR
UPDATE USING (auth.uid () = user_id);

CREATE POLICY "Users can delete own practice sessions" ON public.interview_practice_sessions FOR DELETE USING (auth.uid () = user_id);
...
VALUES (
        'interview_practice_free_mode',
        'true',
        'Skip credit check for interview practice (testing mode)'
    ),
    (
        'interview_practice_cost',
        '1',
        'Credits per practice session when not in free mode'
    ) ON CONFLICT (key) DO NOTHING;

-- 6. Index for performance
CREATE INDEX IF NOT EXISTS idx_interview_practice_user ON public.interview_practice_sessions (user_id, created_at DESC);
...
INSERT INTO
    system_config (key, value, description)
VALUES (
        'rejection_analyzer_free_mode',
        'true',
        'Skip credit check for rejection analysis (testing)'
    ),
    (
        'rejection_analyzer_cost',
        '1',
        'Credits per rejection analysis'
    ) ON CONFLICT (key) DO NOTHING;
...
CREATE POLICY "Users can view own career paths" ON public.career_path_sessions FOR
SELECT USING (auth.uid () = user_id);

CREATE POLICY "Users can insert own career paths" ON public.career_path_sessions FOR
INSERT
WITH
    CHECK (auth.uid () = user_id);

CREATE POLICY "Users can update own career paths" ON public.career_path_sessions FOR
UPDATE USING (auth.uid () = user_id);

CREATE POLICY "Users can delete own career paths" ON public.career_path_sessions FOR DELETE USING (auth.uid () = user_id);

CREATE INDEX IF NOT EXISTS idx_career_path_user ON public.career_path_sessions (user_id, created_at DESC);

INSERT INTO
    system_config (key, value, description)
VALUES (
        'career_path_free_mode',
        'true',
        'Skip credit check for career path (testing)'
    ),
    (
        'career_path_cost',
        '1',
        'Credits per career path generation'
    ) ON CONFLICT (key) DO NOTHING;
...
CREATE POLICY "Users can view own skill gaps" ON public.skill_gap_analyses FOR
SELECT USING (auth.uid () = user_id);

CREATE POLICY "Users can insert own skill gaps" ON public.skill_gap_analyses FOR
INSERT
WITH
    CHECK (auth.uid () = user_id);

CREATE POLICY "Users can update own skill gaps" ON public.skill_gap_analyses FOR
UPDATE USING (auth.uid () = user_id);

CREATE POLICY "Users can delete own skill gaps" ON public.skill_gap_analyses FOR DELETE USING (auth.uid () = user_id);

CREATE INDEX IF NOT EXISTS idx_skill_gap_user ON public.skill_gap_analyses (user_id, created_at DESC);
...
VALUES (
        'skill_gap_free_mode',
        'true',
        'Skip credit check for skill gap (testing)'
    ),
    (
        'skill_gap_cost',
        '1',
        'Credits per skill gap analysis'
    ) ON CONFLICT (key) DO NOTHING;
...
CREATE POLICY "Users can view own alerts" ON public.job_alert_preferences FOR
SELECT USING (auth.uid () = user_id);

CREATE POLICY "Users can insert own alerts" ON public.job_alert_preferences FOR
INSERT
WITH
    CHECK (auth.uid () = user_id);

CREATE POLICY "Users can update own alerts" ON public.job_alert_preferences FOR
UPDATE USING (auth.uid () = user_id);

CREATE POLICY "Users can delete own alerts" ON public.job_alert_preferences FOR DELETE USING (auth.uid () = user_id);
...
CREATE POLICY "Users can view own alert history" ON public.job_alert_history FOR
SELECT USING (auth.uid () = user_id);

CREATE INDEX IF NOT EXISTS idx_job_alert_user ON public.job_alert_preferences (user_id, is_active);

CREATE INDEX IF NOT EXISTS idx_job_alert_history_user ON public.job_alert_history (user_id, sent_at DESC);

INSERT INTO
    system_config (key, value, description)
VALUES (
        'job_alerts_free_mode',
        'true',
        'Skip subscription check for job alerts (testing)'
    ),
    (
        'job_alerts_max_per_user',
        '3',
        'Max alert preferences per user'
    ) ON CONFLICT (key) DO NOTHING;
...
CREATE POLICY "Users can view own auto apply" ON public.auto_apply_settings FOR
SELECT USING (auth.uid () = user_id);

CREATE POLICY "Users can insert own auto apply" ON public.auto_apply_settings FOR
INSERT
WITH
    CHECK (auth.uid () = user_id);

CREATE POLICY "Users can update own auto apply" ON public.auto_apply_settings FOR
UPDATE USING (auth.uid () = user_id);
...
CREATE POLICY "Users can view own auto apply log" ON public.auto_apply_log FOR
SELECT USING (auth.uid () = user_id);

CREATE INDEX IF NOT EXISTS idx_auto_apply_user ON public.auto_apply_settings (user_id, is_active);

CREATE INDEX IF NOT EXISTS idx_auto_apply_log_user ON public.auto_apply_log (user_id, created_at DESC);

INSERT INTO
    system_config (key, value, description)
VALUES (
        'auto_apply_free_mode',
        'true',
        'Skip subscription check for auto apply (testing)'
    ),
    (
        'auto_apply_tier_basic',
        '50',
        'Monthly limit for basic tier'
    ),
    (
        'auto_apply_tier_pro',
        '100',
        'Monthly limit for pro tier'
    ),
    (
        'auto_apply_tier_premium',
        '200',
        'Monthly limit for premium tier'
    ) ON CONFLICT (key) DO NOTHING;
...
-- ملخص AI {"top_skills":[], "top_experiences":[]}