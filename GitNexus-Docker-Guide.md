...
Because the global `gitnexus` alias is not mapped in the official Docker image, you must call the node script directly using `docker exec -u node`.

> **Important:** Always use `-u node` to avoid root permission issues with the DuckDB database.

To analyze the current folder that is mounted as `/workspace`:

**Basic analysis (graph only, no semantic search):**
```bash
docker exec -u node gitnexus-server node /app/gitnexus/dist/cli/index.js analyze /workspace
```

**Full analysis with embeddings (enables semantic/vector search):**
```bash
docker exec -u node gitnexus-server node /app/gitnexus/dist/cli/index.js analyze /workspace --embeddings
```

> **Note:** `--embeddings` is **OFF by default**. Without it, `query` results will only use BM25 keyword matching. With it, GitNexus generates vector embeddings using the `EMBEDDING_MODEL` configured in `docker-compose.yml` (e.g., `qwen3-embedding:8b` via Ollama).

**Force a full re-index (clears cache):**
```bash
docker exec -u node gitnexus-server node /app/gitnexus/dist/cli/index.js analyze /workspace --force --embeddings
```

**If it is NOT a Git repository (or you want to ignore Git history):**
```bash
docker exec -u node gitnexus-server node /app/gitnexus/dist/cli/index.js analyze /workspace --skip-git --embeddings
```
...
```bash
docker exec -u node gitnexus-server git config --global --add safe.directory /workspace
```
...
```
*(If you are using a different model like `mistral` or `gemma4`, change `--model llama3` accordingly)*.

> **Note:** `--base-url`, `--model`, and `--api-key` flags are only for the `wiki` command (LLM generation). The `analyze` command reads embedding config from Docker environment variables (`OPENAI_BASE_URL`, `EMBEDDING_MODEL`, `OPENAI_API_KEY`) set in `docker-compose.yml`.
